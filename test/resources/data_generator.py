...
class Data:
    ...
    def generate_complex_segment(self, n_obs: int, n_features: int):
        ...
        y_complex = (
            (X_complex[:, 0] + X_complex[:, 1] * X_complex[:, 2]) > 0
        ).astype(int)
    ...
    def generate_high_dimensional_segment(self, n_obs: int, n_features: int):
        ...
        """
        X_sparse = np.random.binomial(
            1, 0.1, (n_obs, n_features)
    ...
    def plot_segments(self):
        ...
        titles = [
            ...
            "Imbalanced Segment",
            "Noisy Segment",
            "High-dimensional Sparse Segment",
        ]

        for i, (segment_X, segment_y, title) in enumerate(
            zip(self.segments_X, self.segments_y, titles)
        ):
            row, col = divmod(i, 2)
    ...
    def plot_segments_pca(self):
        ...
        pca_segments_X = [
            pca.fit_transform(segment_X) for segment_X in self.segments_X
        ]
        ...
        for i, (segment_X, segment_y, title) in enumerate(
            zip(pca_segments_X, self.segments_y, titles)
        ):
            row, col = divmod(i, 2)
            axes[row, col].scatter(
                segment_X[:, 0],
